...
function _start() {
   if (recording) return;
   console.log("De NavData wordt opgeslagen");
   ...
   fs.mkdir(path.join(root, folder), function() {
      ...
      motionStream.write("seq,pitch,roll,yaw,xVelocity,yVelocity,zVelocity,altitude\n", function() {
          recording = true;
          io.sockets.emit('/message', "De NavData wordt opgeslagen.");
      });
    });
}
...
function _stop() {
    if (!recording) return;
    console.log("De navdata is opgeslagen");
    recording = false;
    navStream.end();
    motionStream.end();
    paveStream.end();
    videoStream.end();
    io.sockets.emit('/message', "De NavData is opgeslagen.");
}
...
function _writeNavData(data) {
    ...
    console.log('Requested ' + datastring);
